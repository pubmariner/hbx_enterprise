...
describe Soap::V1::PoliciesController do
  ...
  describe "index service" do
    ...
    context "Authentication failure" do
      it "should raise HTTP 401 error" do
        expect { @client.call(:index, message: { "page" => "1", "enrollment_group_id" => "8437414737041424384", "user_token"=>"zUzBsoTSKPbvXCQsB4Ky00" })  }.to raise_error(Savon::HTTPError) { |e| puts expect(e.http.code).to eq(401) }
        expect { @client.call(:index, message: { "page" => "1", "enrollment_group_id" => "8437414737041424384", "user_token"=>"" })  }.to raise_error(Savon::HTTPError) { |e| puts expect(e.http.code).to eq(401) }
    ...
    context "Missing enrollment_group_id" do
      it "should raise HTTP 422 error" do
        ...
        expect { @client.call(:index, message: { "page" => "1", "enrollment_group_id" => "", "user_token"=>"zUzBsoTSKPbvXCQsB4Ky" })  }.to raise_error(Savon::HTTPError) { |e| puts expect(e.http.code).to eq(422) }
      end
    end
  ...
  describe "show service" do
    ...
    context "Authentication failure" do
      it "should raise HTTP 401 error" do
        expect { @client.call(:show, message: { "id" => "1", "user_token" => "zUzBsoTSKPbvXCQsB4Ky00"}) }.to raise_error(Savon::HTTPError) { |e| puts expect(e.http.code).to eq(401) }
        expect { @client.call(:show, message: { "id" => "1", "user_token" => ""}) }.to raise_error(Savon::HTTPError) { |e| puts expect(e.http.code).to eq(401) }
