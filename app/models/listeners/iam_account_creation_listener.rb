...
module Listeners
  class IamAccountCreationListener < Amqp::Client
    def on_message(delivery_info, properties, payload)
      headers = (properties.headers || {})
      code, body = Proxies::IamAccountCreation.new.request(headers.stringify_keys, 10)
      case code.to_s
      when "201"
        # ALL GOOD
      ...
      when "503"
        log_failure("error.events.account_management.iam_creation_timeout",code, headers, body)
        channel.nack(delivery_info.delivery_tag,false, true)
      else
        log_failure("error.events.account_management.iam_creation_failure",code, headers, body)
        channel.acknowledge(delivery_info.delivery_tag, false)
      end
    end
    ...
    def self.queue_name
      ...
      "#{ec.hbx_id}.#{ec.environment}.q.hbx_enterprise.iam_account_creation"
    end
    ...
    def send_response(status, headers, r_body)
      ...
      response_properties = {
        :timestamp => Time.now.to_i,
        :routing_key => "info.events.account_management.iam_creation_success",
        :headers => headers.merge({
