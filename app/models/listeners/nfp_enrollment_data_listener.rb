module Listeners
  class NfpEnrollmentDataListener < Amqp::Client
    ...
    def on_message(delivery_info, properties, payload)
      ...
      when "503"
        log_failure("error.events.employer.nfp_enrollment_data_request_timeout",code, headers, body)
        channel.nack(delivery_info.delivery_tag,false, true)
      else
        log_failure("error.events.employer.nfp_enrollment_data_request_failure",code, headers, body)
        channel.acknowledge(delivery_info.delivery_tag, false)
      end
    end
